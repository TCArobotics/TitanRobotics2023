...
public class TeleopControl
{
    ...
    double AutoBalTeleSetUp = 1;
    boolean autoBalanceOn = false;

    public TeleopControl(DriveControl driveControl, Claw claw, Arm arm, AHRS ahrs)
    {
        ...
        this.gyroDrive = new GyroDrive(ahrs);
    }
    ...
    public void AutoBalDriveControl()
    {
        boolean buttonApressed = xboxGamePad.getButtonXboxPressed(ButtonMap.XboxA);
        boolean buttonBpressed = xboxGamePad.getButtonXboxPressed(ButtonMap.XboxB);
        
        if(buttonApressed && !autoBalanceOn)
        {
           autoBalanceOn = true;
        }
        else if(buttonApressed && autoBalanceOn)
        {
            autoBalanceOn = false;
        }
        if(autoBalanceOn)
        {
            this.gyroDrive.autoBalance();
        }
    }
    ...
    public void armPivotControl() //controls the pivoting of the arm (Flight controller stick y-axis)
    {
        ...
        this.arm.runPivotMotor(-0.5 * flightYInput);
    }
